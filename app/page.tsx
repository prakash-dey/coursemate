...
import { isSendShortcut } from "@/lib/chat-input";
import { orderConversation } from "@/lib/conversation-history";
...
export default function Home() {
  ...
  const [uploading, setUploading] = useState(false); const [quiz, setQuiz] = useState<QuizQuestion[] | null>(null); const [quizIndex, setQuizIndex] = useState<number | null>(null); const [quizBusy, setQuizBusy] = useState(false); const [answers, setAnswers] = useState<Record<number, number>>({});
  ...
  const conversationHistory = useMemo(() => orderConversation(messages, quiz, quizIndex), [messages, quiz, quizIndex]);
  ...
  async function ask(event: FormEvent) {
    event.preventDefault(); if (!selected || !question.trim()) return; const text = question.trim(); setQuestion(""); setAsking(true); setError(""); setMessages((items) => [...items, { role: "user", text }]);
  ...
  async function makeQuiz() {
    if (!selected) return; const insertionIndex = messages.length; setQuizBusy(true); setQuiz(null); setQuizIndex(null); setAnswers({}); setError("");
    try { const response = await fetch("/api/quiz", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ courseId: selected.id, topic: "key concepts" }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); setQuiz(data.quiz); setQuizIndex(insertionIndex); }
  ...
  return <main className="platform-shell">
    <aside className="sidebar"><div className="brand"><span className="brand-mark">C</span><span>CourseMate</span></div><div className="side-label">YOUR COURSES</div><nav>{courses.map((course) => <button className={course.id === selectedId ? "course-link active" : "course-link"} key={course.id} onClick={() => { setSelectedId(course.id); setMessages([]); setQuiz(null); setQuizIndex(null); }}><span>{course.name.slice(0, 1).toUpperCase()}</span><div><strong>{course.name}</strong><small>{course.documents.length} document{course.documents.length === 1 ? "" : "s"}</small></div></button>)}</nav><details className="new-course" open={!courses.length}><summary>＋ New course</summary><form onSubmit={createCourse}><label>Course name<input name="name" maxLength={100} required placeholder="e.g. Distributed Systems" /></label><label>Description<textarea name="description" maxLength={500} placeholder="What are you learning?" /></label><button disabled={creating}>{creating ? "Creating…" : "Create course"}</button></form></details><form action={signOut} className="sign-out"><button>Sign out</button></form></aside>
    {!selected ? <section className="empty-workspace"><div className="empty-icon">↗</div><p className="overline">START YOUR LIBRARY</p><h1>Create your first course.</h1><p>Give your learning space a name, then add the PDFs or notes you want CourseMate to use.</p></section> : <section className="course-workspace"><header><div><p className="overline">COURSE WORKSPACE</p><h1>{selected.name}</h1><p>{selected.description || "Your private, source-grounded study space."}</p></div><button className="quiz-button" disabled={!readyCount || quizBusy} onClick={makeQuiz}>{quizBusy ? "Building quiz…" : "Quiz this course"}</button></header>
      <div className="workspace-grid"><section className="study-pane"><div className="materials"><div className="section-heading"><div><h2>Course material</h2><p>{readyCount} ready · {selected.documents.length} total</p></div><form onSubmit={upload}><label className={uploading ? "upload-button busy" : "upload-button"}>＋ {uploading ? "Processing…" : "Add document"}<input type="file" name="file" accept=".pdf,.txt,.md,application/pdf,text/plain,text/markdown" required disabled={uploading} onChange={(event) => event.currentTarget.form?.requestSubmit()} /></label></form></div>{!selected.documents.length ? <div className="material-empty"><strong>No material yet</strong><p>Upload a PDF, Markdown, or text file. CourseMate will parse, chunk, and embed it for this course.</p></div> : <div className="document-list">{selected.documents.map((doc) => <div className="document-row" key={doc.id}><div className="file-icon">{doc.title.toLowerCase().endsWith(".pdf") ? "PDF" : "TXT"}</div><div><strong>{doc.title}</strong><small>{doc.status === "ready" ? `${doc.chunk_count} searchable sections` : doc.status === "failed" ? doc.error_message || "Processing failed" : "Parsing and embedding…"}</small></div><span className={`status ${doc.status}`}>{doc.status}</span></div>)}</div>}</div>
        <div className="conversation" aria-live="polite">{!conversationHistory.length && <div className="chat-empty"><p className="overline">ASK YOUR MATERIAL</p><h2>{readyCount ? "What do you want to understand?" : "Add material to begin."}</h2><p>{readyCount ? "Answers use only the ready documents in this course and always show their sources." : "Questions and quizzes unlock when at least one document finishes processing."}</p></div>}{conversationHistory.map((entry, index) => entry.kind === "message" ? <article className={`message ${entry.value.role}`} key={`message-${index}`}><div className="message-label">{entry.value.role === "user" ? "YOU" : "COURSEMATE"}</div><div className="message-body"><p>{entry.value.text}</p>{entry.value.result && entry.value.result.mode !== "conversation" && <><div className={`grounding ${entry.value.result.grounded ? "found" : "missing"}`}>{entry.value.result.grounded ? `Grounded in ${entry.value.result.sources.length} source${entry.value.result.sources.length === 1 ? "" : "s"}` : "No evidence found"}</div>{entry.value.result.sources.map((source) => <details className="source" key={`${source.id}-${source.snippet}`}><summary><span>{source.title}</span><b>{source.score}% match</b></summary><p>{source.snippet}</p></details>)}</>}</div></article> : <div className="quiz-sheet" key="course-quiz"><p className="overline">COURSE QUIZ</p><h2>Check your understanding</h2>{entry.value.map((item, qi) => <fieldset key={item.question}><legend>{qi + 1}. {item.question}</legend>{item.options.map((option, oi) => { const answered = answers[qi] !== undefined; return <button key={option} disabled={answered} className={answered && oi === item.answerIndex ? "correct" : answered && answers[qi] === oi ? "wrong" : ""} onClick={() => setAnswers((value) => ({ ...value, [qi]: oi }))}>{String.fromCharCode(65 + oi)}. {option}</button>; })}{answers[qi] !== undefined && <p>{item.explanation}</p>}</fieldset>)}</div>)}{asking && <div className="retrieving"><span className="spinner" />Retrieving only from {selected.name}…</div>}{error && <div className="error" role="alert">{error}</div>}</div>
        <form className="composer" onSubmit={ask}><textarea value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={handleComposerKeyDown} maxLength={500} disabled={!readyCount || asking} placeholder={readyCount ? `Ask ${selected.name}…` : "Waiting for course material…"} aria-label="Ask this course" /><button type="submit" disabled={!readyCount || asking || !question.trim()} aria-label="Send question" title="Send question (Command or Control + Enter)">↑</button><small>⌘/Ctrl + Enter to send · {question.length}/500</small></form></section>
