...
export async function GET() {
  ...
  const { data, error } = await supabase.from("courses").select("id,name,description,created_at,documents(id,title,status,error_message,chunk_count,created_at)").order("created_at", { ascending: false });
...
export async function POST(request: Request) {
  ...
  const { supabase, user } = await requireUser(); if (!user) return NextResponse.json({ error: "Authentication required." }, { status: 401 });
  try { if (!await consumeRateLimit(supabase, "course_create")) return NextResponse.json({ error: "Course creation limit reached. Try again later." }, { status: 429 }); }
