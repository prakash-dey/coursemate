...
export async function POST(request: Request) {
  ...
  try {
    const vector = await embedQuery(parsed.data.topic || "key concepts"); const { data } = await supabase.rpc("match_course_chunks", { query_embedding: vector, target_course_id: course.id, match_threshold: 0.25, match_count: 8 });
