...
export async function proxy(request: NextRequest) {
  ...
  const { data: { user } } = await supabase.auth.getUser(); const isAuthPage = request.nextUrl.pathname.startsWith("/auth");
  if (!user && !isAuthPage) return NextResponse.redirect(new URL("/auth", request.url)); if (user && isAuthPage) return NextResponse.redirect(new URL("/", request.url)); return response;
}
